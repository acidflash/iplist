...
export function Dashboard() {
  ...
  return (
    <div className="p-6 max-w-5xl">
      ...
      <div
        className="flex items-center gap-6 px-4 py-3 rounded-lg mb-6 text-[13px]"
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          {recent.map(p => (
            ...
            >
              <code className="font-ip text-c-accent flex-shrink-0" style={{ fontSize: '12px' }}>
                {p.prefix}
              </code>
              <span className="text-c-text2 flex-1 truncate" style={{ fontSize: '12px' }}>
                {p.name || '–'}
              </span>
...
function Panel({ title, emptyText, isEmpty, children }: {
  ...
  return (
    <div
      ...
      style={{ background: 'var(--c-surface)', border: '1px solid var(--c-border-sub)' }}
    >
      <h2 className="text-[12px] font-semibold text-c-text3 uppercase tracking-wider mb-3">
        {title}
      </h2>
      {isEmpty ? (
        <p className="text-c-text3 py-4 text-center" style={{ fontSize: '12px' }}>{emptyText}</p>
      ) : children}
    </div>
  )
}
